...
public async Task BroadcastDetectionEventAsync(LiveDetectionEvent detectionEvent)
 ...
 _logger.LogDebug("Broadcasted detection event: {IsAttack} from {SrcIp}", 
    detectionEvent.IsAttack ? "Attack" : "Benign", detectionEvent.SrcIp);
            }
            catch (Exception ex)
            {
  ...
  await _hubContext.Clients.Group("Support").SendAsync("AttackAlert", alertData);
        ...
        _logger.LogInformation("Broadcasted attack alert: {AttackType} ({Severity}) from {SrcIp}", 
             attack.AttackType, attack.Severity, attack.SrcIp);
        }
            catch (Exception ex)
            {
