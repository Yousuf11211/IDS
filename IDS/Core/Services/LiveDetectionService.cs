...
namespace IDS.Core.Services
{
    ...
    /// The external detection pipeline writes to RawPackets, Benign_Table and Attack_Table.
...
totalAttacks = await _db.AttackTraffic.LongCountAsync();
      ...
      benign24h = await _db.BenignTraffic.LongCountAsync(t => t.Timestamp >= last24h);
               attacks24h = await _db.AttackTraffic.LongCountAsync(t => t.Timestamp >= last24h);
   
                benignHour = await _db.BenignTraffic.LongCountAsync(t => t.Timestamp >= lastHour);
         attacksHour = await _db.AttackTraffic.LongCountAsync(t => t.Timestamp >= lastHour);
            }
    catch (Exception ex)
 {
  ...
  IsAttack = true,
   ...
   events.AddRange(benign);
       events.AddRange(attacks);
        events = events.OrderByDescending(e => e.Timestamp).Take(count).ToList();
     }
  catch (Exception ex)
       {
